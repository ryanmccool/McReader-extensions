...
var _Sources = (() => {
  ...
  // src/MangaTaro/MangaTaro.ts
  var MangaTaro_exports = {};
  __export(MangaTaro_exports, {
    MangaTaro: () => MangaTaro,
    MangaTaroInfo: () => MangaTaroInfo
  });
  ...
  function md5(str) {
    ...
    function au(x2, y) {
      ...
      return (x2 >> 16) + (y >> 16) + (l >> 16) << 16 | l & 65535;
    }
    function cmn(q, a2, b2, x2, s, t) {
      return au(rl(au(au(a2, q), au(x2, t)), s), b2);
    }
    function ff(a2, b2, c2, d2, x2, s, t) {
      return cmn(b2 & c2 | ~b2 & d2, a2, b2, x2, s, t);
    }
    function gg(a2, b2, c2, d2, x2, s, t) {
      return cmn(b2 & d2 | c2 & ~d2, a2, b2, x2, s, t);
    }
    function hh(a2, b2, c2, d2, x2, s, t) {
      return cmn(b2 ^ c2 ^ d2, a2, b2, x2, s, t);
    }
    function ii(a2, b2, c2, d2, x2, s, t) {
      return cmn(c2 ^ (b2 | ~d2), a2, b2, x2, s, t);
    }
    function toBlocks(s) {
      const n = s.length;
      const blks = [];
      for (let i = 0; i < n * 8; i += 8) blks[i >> 5] |= (s.charCodeAt(i / 8) & 255) << i % 32;
  ...
  function generateToken() {
    ...
    const hour = (/* @__PURE__ */ new Date()).toISOString().slice(0, 13).replace(/[-T:]/g, "");
    const token = md5(String(timestamp) + "mng_ch_" + hour).substring(0, 16);
  ...
  var MangaTaro = class {
    ...
    slugFromPermalink(permalink) {
      const m = (permalink || "").split("/manga/")[1];
      return m ? m.replace(/\/+$/, "") : permalink;
    }
    cleanTitle(name) {
      let t = (name || "").split("|")[0].trim();
      t = t.replace(/\s+(Manga|Manhwa|Manhua|Comic)$/i, "").trim();
      return t;
    }
    // Best-effort relative-date ("2 years ago") → Date; falls back to now.
    parseDate(s) {
      const now = Date.now();
    ...
    // MARK: details (scrape /manga/<slug> JSON-LD)
    async getMangaDetails(mangaId) {
      const html = await this.getText(`${SITE}/manga/${mangaId}`);
      const $ = this.cheerio.load(html);
      let ld = {};
      $('script[type="application/ld+json"]').toArray().forEach((el) => {
        try {
          const j = JSON.parse($(el).html() || "");
          if (j && j["@type"] === "ComicSeries") ld = j;
        } catch (e) {
        }
      });
      const author = ld.author && ld.author.name ? ld.author.name : "";
      const tags = ld.genre ? [App.createTagSection({ id: "genres", label: "Genres", tags: [App.createTag({ id: String(ld.genre), label: String(ld.genre) })] })] : [];
      return App.createSourceManga({
        id: mangaId,
        mangaInfo: App.createMangaInfo({
          titles: [this.cleanTitle(ld.name) || mangaId],
          image: ld.image || "",
    ...
    // MARK: chapters (signed /auth/manga-chapters)
    async getChapters(mangaId) {
      const html = await this.getText(`${SITE}/manga/${mangaId}`);
      const $ = this.cheerio.load(html);
      const numericId = $("[data-manga-id]").first().attr("data-manga-id");
      if (!numericId) return [];
      const chapters = [];
      const limit = 500;
      let offset = 0;
      ...
      while (hasMore) {
        ...
        const url = `${SITE}/auth/manga-chapters?manga_id=${numericId}&offset=${offset}&limit=${limit}&order=ASC&_t=${token}&_ts=${timestamp}`;
        ...
        hasMore = !!json.has_more && list.length > 0;
        offset += limit;
        if (offset > 2e4) break;
      }
      return chapters;
    ...
    async getChapterDetails(mangaId, chapterId) {
      const json = await this.getJSON(`${SITE}/auth/chapter-content?chapter_id=${chapterId}`);
      const pages = (json.images || []).filter((u) => typeof u === "string" && u.length > 0);
      return App.createChapterDetails({ id: chapterId, mangaId, pages });
    }
    // MARK: search + browse
    async getSearchResults(query, metadata) {
      var _a;
      const title = (_a = query == null ? void 0 : query.title) != null ? _a : "";
      const nonceJson = await this.getJSON(`${SITE}/wp-json/manga/v1/refresh-nonce`);
      const list = await this.postJSON(`${SITE}/wp-json/manga/v1/search`, {
        query: title,
        nonce: nonceJson.nonce
      });
      const results = (Array.isArray(list) ? list : []).map(
        (m) => App.createPartialSourceManga({ mangaId: m.slug, image: m.thumbnail, title: m.title })
      );
      return App.createPagedResults({ results, metadata: void 0 });
    }
    async getHomePageSections(sectionCallback) {
      const defs = [
        { id: "popular", title: "Popular", path: "/wp-json/manga/v1/popular" },
        { id: "highscore", title: "Top Rated", path: "/wp-json/manga/v1/highscore" }
      ];
      for (const def of defs) {
        ...
        const list = await this.getJSON(`${SITE}${def.path}`);
        section.items = (Array.isArray(list) ? list : []).map(
          (m) => App.createPartialSourceManga({
            mangaId: this.slugFromPermalink(m.permalink),
            image: m.cover,
            title: m.title
          })
        );
  ...
  return __toCommonJS(MangaTaro_exports);
})();
