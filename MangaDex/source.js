...
var _Sources = (() => {
  ...
  var require_Source = __commonJS({
    "node_modules/@paperback/types/lib/base/Source.js"(exports) {
      ...
      var Source = class {
        ...
        async getTags() {
          var _a;
          return (_a = this.getSearchTags) == null ? void 0 : _a.call(this);
        }
      };
      ...
      function convertTime(timeAgo) {
        var _a;
        let time;
        let trimmed = Number(((_a = /\d*/.exec(timeAgo)) != null ? _a : [])[0]);
  ...
  var MangaDexInfo = {
    ...
    author: "McReader",
    authorWebsite: "https://github.com/ryanmccool",
    icon: "icon.png",
  ...
  var MangaDex = class {
    ...
    async getJSON(url) {
      ...
      const raw = typeof response.data === "string" ? response.data : `${response.data}`;
      return JSON.parse(raw);
    }
    ratings() {
      return RATINGS.map((r) => `contentRating[]=${r}`).join("&");
    }
    // English-preferred value from a localized { lang: value } object.
    pick(obj) {
    ...
    coverURL(mangaId, relationships) {
      ...
      const file = (_a = art == null ? void 0 : art.attributes) == null ? void 0 : _a.fileName;
      return file ? `${COVERS}/${mangaId}/${file}.512.jpg` : "";
    }
    statusLabel(s) {
      switch (s) {
        case "ongoing":
    ...
    getMangaShareUrl(mangaId) {
      return `${SITE}/title/${mangaId}`;
    }
    // MARK: details
    async getMangaDetails(mangaId) {
      var _a, _b, _c, _d;
      const json = await this.getJSON(
        `${API}/manga/${mangaId}?includes[]=cover_art&includes[]=author&includes[]=artist`
      );
      ...
      const author = ((_b = (_a = rels.find((r) => r.type === "author")) == null ? void 0 : _a.attributes) == null ? void 0 : _b.name) || "";
      const artist = ((_d = (_c = rels.find((r) => r.type === "artist")) == null ? void 0 : _c.attributes) == null ? void 0 : _d.name) || "";
      const tags = (attr.tags || []).map(
        (t) => {
          var _a2;
          return App.createTag({ id: t.id, label: this.pick((_a2 = t.attributes) == null ? void 0 : _a2.name) });
        }
      );
      return App.createSourceManga({
        id: mangaId,
        mangaInfo: App.createMangaInfo({
          titles: titles.length ? titles : [mangaId],
          image: this.coverURL(mangaId, rels),
          status: this.statusLabel(attr.status),
          author,
          artist,
          desc: this.pick(attr.description),
    ...
    async getChapters(mangaId) {
      ...
      let index = 0;
      do {
        const url = `${API}/manga/${mangaId}/feed?translatedLanguage[]=${LANG}&${this.ratings()}&order[chapter]=asc&order[volume]=asc&limit=${limit}&offset=${offset}&includes[]=scanlation_group`;
        const json = await this.getJSON(url);
        total = (_a = json.total) != null ? _a : 0;
        const batch = json.data || [];
        for (const c of batch) {
          const a = c.attributes || {};
          const num = parseFloat(a.chapter);
          const group = ((_c = (_b = (c.relationships || []).find(
            (r) => r.type === "scanlation_group"
          )) == null ? void 0 : _b.attributes) == null ? void 0 : _c.name) || "";
          chapters.push(
            App.createChapter({
              id: c.id,
              name: a.title && a.title.length ? a.title : isNaN(num) ? "Oneshot" : `Chapter ${a.chapter}`,
              chapNum: isNaN(num) ? 0 : num,
              volume: a.volume ? parseFloat(a.volume) || 0 : 0,
              time: a.publishAt ? new Date(a.publishAt) : /* @__PURE__ */ new Date(),
              sortingIndex: index++,
        ...
        offset += limit;
      } while (offset < total && offset < 1e4);
      return chapters;
    }
    // MARK: pages
    async getChapterDetails(mangaId, chapterId) {
      var _a, _b;
      const json = await this.getJSON(`${API}/at-home/server/${chapterId}`);
      const baseUrl = json.baseUrl;
      const hash = (_a = json.chapter) == null ? void 0 : _a.hash;
      const files = ((_b = json.chapter) == null ? void 0 : _b.data) || [];
      const pages = files.map((f) => `${baseUrl}/data/${hash}/${f}`);
      return App.createChapterDetails({ id: chapterId, mangaId, pages });
    }
    // MARK: search + browse
    listURL(order, query, offset, limit) {
      const titleParam = query ? `title=${encodeURIComponent(query)}&` : "";
      return `${API}/manga?${titleParam}${order}&${this.ratings()}&availableTranslatedLanguage[]=${LANG}&hasAvailableChapters=true&includes[]=cover_art&limit=${limit}&offset=${offset}`;
    }
    async listPage(order, query, page) {
      var _a;
      const limit = 20;
      const offset = (page - 1) * limit;
      const json = await this.getJSON(this.listURL(order, query, offset, limit));
      const results = (json.data || []).map((m) => this.partial(m));
      const total = (_a = json.total) != null ? _a : 0;
      return App.createPagedResults({
        results,
        metadata: offset + limit < total ? { page: page + 1 } : void 0
      });
    }
    async getSearchResults(query, metadata) {
      var _a, _b;
      const page = (_a = metadata == null ? void 0 : metadata.page) != null ? _a : 1;
      const title = (_b = query == null ? void 0 : query.title) != null ? _b : "";
      const order = title ? "order[relevance]=desc" : "order[followedCount]=desc";
      return this.listPage(order, title, page);
    }
    async getHomePageSections(sectionCallback) {
      const defs = [
        { id: "popular", title: "Popular", order: "order[followedCount]=desc" },
        { id: "latest", title: "Latest Updates", order: "order[latestUploadedChapter]=desc" }
      ];
      for (const def of defs) {
        ...
        const json = await this.getJSON(this.listURL(def.order, "", 0, 20));
        section.items = (json.data || []).map((m) => this.partial(m));
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      var _a;
      const page = (_a = metadata == null ? void 0 : metadata.page) != null ? _a : 1;
      const order = homepageSectionId === "popular" ? "order[followedCount]=desc" : "order[latestUploadedChapter]=desc";
      return this.listPage(order, "", page);
    }
  };
