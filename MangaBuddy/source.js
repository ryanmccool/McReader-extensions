...
var _Sources = (() => {
  ...
  // src/BuddyComplexParser.ts
  var BuddyComplexParser = class {
    constructor() {
      this.parseViewMore = ($) => {
        const mangas = [];
        const collectedIds = [];
  ...
  var BuddyComplex = class {
    constructor(cheerio) {
      ...
      this.parser = new BuddyComplexParser();
    }
    ...
    async getSearchResults(query, metadata) {
      ...
      const url = new URLBuilder(this.baseUrl).addPathComponent("search").addQueryParameter("page", page).addQueryParameter("q", encodeURI(query?.title || "")).buildUrl() + query.includedTags?.map((x) => `&genre%5B%5D=${x.id}`).join("");
      const request = App.createRequest({
        url,
        method: "GET"
      });
    ...
    async getHomePageSections(sectionCallback) {
      const section1 = App.createHomeSection({ id: "hot_updates", title: "Hot Updates", type: import_types.HomeSectionType.singleRowNormal, containsMoreItems: true });
      const section2 = App.createHomeSection({ id: "latest_update", title: "Latest Updates", type: import_types.HomeSectionType.singleRowNormal, containsMoreItems: true });
      const section3 = App.createHomeSection({ id: "top_today", title: "Top Today", type: import_types.HomeSectionType.singleRowNormal, containsMoreItems: true });
      const section4 = App.createHomeSection({ id: "top_weekly", title: "Top Weekly", type: import_types.HomeSectionType.singleRowNormal, containsMoreItems: true });
      const section5 = App.createHomeSection({ id: "top_monthly", title: "Top Monthly", type: import_types.HomeSectionType.singleRowNormal, containsMoreItems: true });
...
Please go to Settings > Sources > ${this.baseUrl} and press Cloudflare Bypass`);
  ...
  // src/MangaBuddy/MangaBuddyParser.ts
  var MangaBuddyParser = class extends BuddyComplexParser {
    fullParseChapterList($, mangaId, mangaPage$) {
      const chapters = super.parseChapterList($, mangaId);
      if (chapters.length === 0) {
        return chapters;
      }
      const lastChapters = super.parseChapterList(mangaPage$, mangaId);
  ...
  var MangaBuddy = class extends BuddyComplex {
    constructor() {
      super(...arguments);
      this.baseUrl = DOMAIN;
      this.parser = new MangaBuddyParser();
    }
    async getHomePageSections(sectionCallback) {
      const section1 = App.createHomeSection({ id: "hot_updates", title: "Hot Updates", type: import_types2.HomeSectionType.singleRowNormal, containsMoreItems: true });
      const section2 = App.createHomeSection({ id: "latest_update", title: "Latest Updates", type: import_types2.HomeSectionType.singleRowNormal, containsMoreItems: true });
      const section3 = App.createHomeSection({ id: "top_today", title: "Top Today", type: import_types2.HomeSectionType.singleRowNormal, containsMoreItems: true });
      const section4 = App.createHomeSection({ id: "top_weekly", title: "Top Weekly", type: import_types2.HomeSectionType.singleRowNormal, containsMoreItems: true });
      const section5 = App.createHomeSection({ id: "top_monthly", title: "Top Monthly", type: import_types2.HomeSectionType.singleRowNormal, containsMoreItems: true });
      const sections = [section1, section2, section3, section4, section5];
      const request = App.createRequest({
    ...
    async getChapters(mangaId) {
      ...
      const mangaPage$ = this.cheerio.load(responseMangaPage.data);
      return this.parser.fullParseChapterList($, mangaId, mangaPage$);
    }
    async getNumericMangaId(mangaId) {
